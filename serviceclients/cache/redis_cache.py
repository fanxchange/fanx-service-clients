...
class RedisCache(object):
    ...
    def read_connection(self):
        ...
        if not self.read_conn:
            try:
                self.read_conn = StrictRedis(host=self.config['read_host'],
                                             port=self.config['read_port'],
    ...
    def write_connection(self):
        ...
        if not self.write_conn:
            try:
                self.write_conn = StrictRedis(host=self.config['write_host'],
                                              port=self.config['write_port'],
                                              socket_keepalive=False,
                                              retry_on_timeout=True,
                                              socket_timeout=self.REQUEST_TIMEOUT_SECS,
                                              socket_connect_timeout=self.CONNECT_TIMEOUT_SECS)
